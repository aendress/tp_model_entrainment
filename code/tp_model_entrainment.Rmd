...
expand.grid(
    ...
    stat_summary(fun.data = mean_se,
                 geom = "pointrange", color = "#cc556f") +
    facet_wrap(. ~ l.act, scales = "free_y") -> plot.basic.weight.in.2syll
...
dat.all.activations.basic.m.wide.m.by.pos <- dat.all.activations.basic.m.wide.m %>% 
    # don't directly use the orginal long version so the same number of trials is removed as above
...
dat.all.activations.basic.m.wide.m.by.pos %>%     
    ...
    geom_violin (alpha = .5,
                 fill = "#5588CC",
                 col="#5588CC") +
    scale_x_discrete(TeX ("Coactivated syllable type"),
                     labels = ~ TeX(paste0("$\\sigma_", .x)),
                     guide = guide_axis(angle = 0)) +
